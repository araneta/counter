...
<?php
$handle = @fopen("app/gammu/smsdrc1", "r");
$baris = array();
if ($handle) {
    while (!feof($handle)) {
        $buffer = fgets($handle);
...
$handle = @fopen("app/gammu/smsdrc2", "r");
$baris = array();
if ($handle) {
    while (!feof($handle)) {
        $buffer = fgets($handle);
...
$handle = @fopen("app/gammu/smsdrc3", "r");
$baris = array();
if ($handle) {
    while (!feof($handle)) {
        $buffer = fgets($handle);
...
$handle = @fopen("app/gammu/smsdrc4", "r");
$baris = array();
if ($handle) {
    while (!feof($handle)) {
        $buffer = fgets($handle);
...
if (@$_GET['op'] == "simpan")
{
  ...
  $handle = @fopen("app/gammu/smsdrc1", "w");
  $text = "[gammu]
...
database = ".$db.";\n";
  ...
  $handle = @fopen("app/gammu/smsdrc2", "w");
  $text = "[gammu]
# isikan no port di bawah ini
port = ".$port2.":
# isikan jenis connection di bawah ini
...
database = ".$db."\n";  
  ...
  $handle = @fopen("app/gammu/smsdrc3", "w");
  $text = "[gammu]
# isikan no port di bawah ini
port = ".$port3.":
# isikan jenis connection di bawah ini
...
database = ".$db."\n";  
  ...
  $handle = @fopen("app/gammu/smsdrc4", "w");
  $text = "[gammu]
# isikan no port di bawah ini
port = ".$port4.":
# isikan jenis connection di bawah ini
...
<form method="post" action="<?php $_SERVER['PHP_SELF']; ?>?page=step4&op=simpan">
<table>
