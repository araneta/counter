...
if ($handle) {
    ...
    fclose($handle);
		 $data= array('host' => 'localhost','user' => $user,'pass' => $pass,'db' => $db);
	return $data;
}
}
...
function get_date_now2() {
    return date("-m-d");
}
function get_date_now3() {
     return date("Y-m-d H:i:s");
}
function get_date_nows() {
     return date("Y-m-d");
}
function get_date_now4() {
    return date("Y-m-");
}
function jatuhtempo($tgl) {
    $new = null;
    $tgl = explode("/", $tgl);
	$tahun=date("Y");
	$tahun2=substr($tahun, 0, 2);
    if (empty($tgl[0]))
        return "";
    $new = "$tahun2$tgl[1]-$tgl[0]-01";
    return $new;
}
/**
...
function lembar_header_excel($colspan){
 ...
 function RandUnik($panjang) { 
   $pstring = "A123B456C789D0FGHIJKLrizaldymulyatno@gmail.comMNOPQRSTUVWXYZ#abcdefghijklmnopqrstuvwxyz"; 
   $plen = strlen($pstring); 
   $unik='';
      for ($i = 1; $i <= $panjang; $i++) { 
          $start = rand(1,$plen); 
...
function RandUdik($panjangd) { 
   $pstringd = "0987654321abcdefghijklmnopqrstuvwxyzA@BCDEFGHIJKLMNOPQRTUVWXZY"; 
   $plend = strlen($pstringd); 
   $udik='';
      for ($i = 1; $i <= $panjangd; $i++) { 
          $startd = rand(1,$plend); 
          $udik.= substr($pstringd, $startd, 1); 
      } 
   return $udik; 
} 
function new_id_member($lastid) {
    $new = null;
    $pecah = explode("001CUST", $lastid);
	$new = "001CUST";
    if ($lastid=="001CUST12040030"){
    $new .= $pecah['1']+100;
	}
	else{
	 $new .= $pecah['1']+1;
...
function new_non_member($lastid) {
    $new = null;
	$new = "001NON00";
	 $new .= $lastid+1;
    return $new;
...
function aktivasi_muat_data($panjang) {
$var1=RandUnik($panjang);
$var2=RandUdik($panjang);
$hasil = md5($var1.'+'.$var2.'rabiha_pilar_informatika_^_^!@~');
//$acak1 = md5($hasil); // nilai acak 1
$acak2=substr($hasil, 0, 6);
$strtampil = $acak2;
return $strtampil ;
}